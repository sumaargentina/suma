...
export function InsurancesTab() {
    ...
    useEffect(() => {
        const loadData = async () => {
            if (user?.id) {
                try {
                    setLoading(true);
                    // Asumimos que el user.id es el clinicId o está vinculado
                    // En la implementación actual, getClinic usa el ID directo.
                    // Si el usuario es 'clinic', su ID es el ID de la clínica.
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            await supabaseService.updateClinic(clinic.id, {
    ...
    if (!clinic && loading) return <div>Cargando...</div>;
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Administra las Obras Sociales y Prepagas que acepta tu clínica. Esta información será visible para los pacientes.
                </CardDescription>
            </CardHeader>
