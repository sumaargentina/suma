-- =====================================================
-- MIGRACIÃ“N 006: HISTORIA DE SALUD Y BIENESTAR (HCE)
-- DescripciÃ³n: Tablas para gestionar el historial de pacientes/clientes
-- Adaptado para: MÃ©dicos, Especialistas, Spas y Centros de Bienestar
-- Fecha: 2025-12-14
-- =====================================================

-- 1. TABLA DE ANTECEDENTES Y PREFERENCIAS
-- Sirve para: Enfermedades (MÃ©dico) o Condiciones/Preferencias (Spa)
-- Ej: "Diabetes" (MÃ©dico) o "Piel Sensible" (Spa)
CREATE TABLE IF NOT EXISTS public.patient_conditions (
    ...
    created_by UUID -- Profesional que lo registrÃ³
);

-- 2. TABLA DE ALERGIAS Y CONTRAINDICACIONES
-- Ej: "Penicilina" (MÃ©dico) o "Alergia a Lavanda" (Spa)
...
-- 3. TABLA DE ANTECEDENTES FAMILIARES (Opcional para Spa, vital para mÃ©dicos)
...
-- 4. TABLA PRINCIPAL DE REGISTROS (NOTAS DE EVOLUCIÃ“N / SESIONES)
CREATE TABLE IF NOT EXISTS public.medical_records (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    patient_id UUID NOT NULL REFERENCES public.patients(id) ON DELETE CASCADE,
    doctor_id UUID NOT NULL REFERENCES public.doctors(id) ON DELETE RESTRICT, -- "Doctor" aquÃ­ es el "Profesional" (MÃ©dico o Especialista)
    ...
    symptoms TEXT,           -- SÃ­ntomas / Molestias / Necesidades del cliente
    
    diagnosis TEXT,          -- DiagnÃ³stico MÃ©dico / EvaluaciÃ³n EstÃ©tica (Ej: "Contractura muscular leve")
    ...
    -- Datos estructurados (Signos vitales O Medidas corporales para estÃ©tica)
    -- Ej MÃ©dico: { "pressure": "120/80", "weight": 70 }
...
-- 5. TABLA DE ARCHIVOS ADJUNTOS (ESTUDIOS, FOTOS ANTES/DESPUÃ‰S)
...
-- POLÃTICAS SIMPLIFICADAS PARA DESARROLLO (Luego ajustar a roles especÃ­ficos)
...
CREATE POLICY "Authenticated users can view records" ON public.medical_records
    FOR SELECT USING (auth.role() = 'authenticated'); -- TODO: Restringir a dueÃ±o y profesional
